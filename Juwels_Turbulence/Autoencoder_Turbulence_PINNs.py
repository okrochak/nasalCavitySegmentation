...
def main():
    ...
    # load data to GPUs
    dataloader = torch.utils.data.DataLoader(dataset=turb_data, sampler=sampler, num_workers=0, batch_size=BATCH_SIZE, pin_memory=True, shuffle=False, drop_last=True)
    dataloader_test = torch.utils.data.DataLoader(dataset=test_data, sampler=sampler_test, num_workers=0, batch_size=BATCH_SIZE, pin_memory=True, shuffle=False, drop_last=True)
    print('data loaded')
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for data in dataloader:
            ...
            dphi_dx = torch.nn.functional.conv2d(m(predictions), weights_x, groups=3)
            
            weights_z = torch.tensor([[0.,0.,0.],[-1/(2*0.0866),0.,1/(2*0.0866)],[0.,0.,0.]])
            weights_z = weights_z.view(1, 1, 3, 3).repeat(3,1,1,1).to(device)
            dphi_dz = torch.nn.functional.conv2d(m(predictions.permute(0,1,3,2)),
                    weights_z, groups=3).permute(0,1,3,2)
            
            ## for the convolution in y-dir (wall-normal), the tensor has to be transposed first
            predictions = predictions.reshape(BATCH_SIZE,3,3,450,192).permute(0,2,1,3,4).reshape(BATCH_SIZE*3,3,450,192)
            m_y1 = nn.ReplicationPad2d((1,1,0,1))
            m_y2 = nn.ZeroPad2d((0,0,1,0))
            ...
            dphi_dy = torch.nn.functional.conv2d(m_y1(m_y2(predictions.permute(0,2,1,3))), weights_y, groups=450).permute(0,2,1,3).reshape(BATCH_SIZE,3,3,450,192)
            
            dphi_dx = dphi_dx.reshape(BATCH_SIZE,3,3,450,192).permute(0,2,1,3,4)
            dphi_dz = dphi_dz.reshape(BATCH_SIZE,3,3,450,192).permute(0,2,1,3,4)
            
            predictions_pinns = torch.zeros((BATCH_SIZE*3,3,450,192)).to(device)
            predictions_pinns[torch.arange(0,BATCH_SIZE*3,3),:,:,:] = (dphi_dy[:,2,:,:,:]-dphi_dz[:,1,:,:,:])
            predictions_pinns[torch.arange(1,BATCH_SIZE*3,3),:,:,:] = (dphi_dz[:,0,:,:,:]-dphi_dx[:,2,:,:,:])
            predictions_pinns[torch.arange(2,BATCH_SIZE*3,3),:,:,:] = (dphi_dx[:,1,:,:,:]-dphi_dy[:,0,:,:,:])

            loss = loss_function(predictions_pinns.float(), inputs.reshape(BATCH_SIZE,3,3,450,192).permute(0,2,1,3,4).reshape(BATCH_SIZE*3,3,450,192).float().to(device))   # Compute loss function
            # ===================backward====================
    ...
    with torch.no_grad():
        for data in dataloader_test:
            ...
            m = nn.ReplicationPad2d((1,1,1,1))
            weights_x = torch.tensor([[0.,0.,0.],[-1/(2*0.2598),0.,1/(2*0.2598)],[0.,0.,0.]])
            weights_x = weights_x.view(1, 1, 3, 3).repeat(3,3,1,1).to(device)
            dphi_dx = torch.nn.functional.conv2d(m(predictions), weights_x)
            
            weights_z = torch.tensor([[0.,0.,0.],[-1/(2*0.0866),0.,1/(2*0.0866)],[0.,0.,0.]])
            weights_z = weights_z.view(1, 1, 3, 3).repeat(3,3,1,1).to(device)
            dphi_dz = torch.nn.functional.conv2d(m(predictions.permute(0,1,3,2)),
                    weights_z).permute(0,1,3,2)
            ...
            weights_y = torch.tensor([[0.,0.,0.],[-0.0262/(0.0255*(0.0255+0.0262)),
                        (0.0262-0.0255)/(0.0262*0.0255),0.0255/(0.0262*(0.0255+0.0262))],[0.,0.,0.]])
            weights_y = weights_y.view(1,1,3,3).repeat(450,450,1,1).to(device)
            dphi_dy = torch.nn.functional.conv2d(m_y1(m_y2(predictions.permute(0,2,1,3))), weights_y).permute(0,2,1,3).reshape(BATCH_SIZE,3,3,450,192)
            ...
            loss = loss_function(predictions_pinns.float(), inputs.reshape(BATCH_SIZE,3,3,450,192).permute(0,2,1,3,4).reshape(BATCH_SIZE*3,3,450,192).float().to(device))
