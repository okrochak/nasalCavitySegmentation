...
#Encoding layers - conv_en1 denotes 1st (1) convolutional layer for the encoding (en) part
        self.conv_en1 = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, stride=1, padding=1)
        self.bnorm_en1 = nn.BatchNorm2d(16)
        self.conv_en2 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, stride=1, padding=1)
...
#Decoding layers - conv_de1 denotes outermost (1) convolutional layer for the decoding (de) part
        ...
        self.conv_de1 = nn.Conv2d(in_channels=16, out_channels=3, kernel_size=3, stride=1, padding=[1,1])
...
# load datasets
    ...
    turb_data = datasets.DatasetFolder(args.data_dir+'Width1800',\
...
# start trainin loop
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for data in train_loader:
            ...
            weights_x = torch.tensor([[0.,0.,0.],[-1/(2*0.2598),0.,1/(2*0.2598)],[0.,0.,0.]])
            weights_x.requires_grad=True
            weights_x = weights_x.view(1, 1, 3, 3).repeat(3,1,1,1).to(device)
            dphi_dx = torch.nn.functional.conv2d(predictions, weights_x, groups=3)

            dphi_dx = dphi_dx.reshape(args.batch_size,3,3,448,190).permute(0,2,1,3,4)
            dphi_dx = dphi_dx[:,:,1:-1,:,:]
            
            weights_z = torch.tensor([[0.,0.,0.],[-1/(2*0.0866),0.,1/(2*0.0866)],[0.,0.,0.]])
            weights_z.requires_grad=True
            weights_z = weights_z.view(1, 1, 3, 3).repeat(3,1,1,1).to(device)
            dphi_dz = torch.nn.functional.conv2d(predictions.permute(0,1,3,2), weights_z, groups=3).permute(0,1,3,2)
            
            dphi_dz = dphi_dz.reshape(args.batch_size,3,3,448,190).permute(0,2,1,3,4)
            dphi_dz = dphi_dz[:,:,1:-1,:,:]
            
            ## for the convolution in y-dir (wall-normal), the tensor has to be transposed first
            predictions = predictions.reshape(args.batch_size,3,3,450,192).permute(0,2,1,3,4).reshape(args.batch_size*3,3,450,192)
            weights_y = torch.tensor([[-0.0262/(0.0255*(0.0255+0.0262)),
            (0.0262-0.0255)/(0.0262*0.0255),0.0255/(0.0262*(0.0255+0.0262))],[-0.0262/(0.0255*(0.0255+0.0262)),
                        (0.0262-0.0255)/(0.0262*0.0255),0.0255/(0.0262*(0.0255+0.0262))],[-0.0262/(0.0255*(0.0255+0.0262)),
                        (0.0262-0.0255)/(0.0262*0.0255),0.0255/(0.0262*(0.0255+0.0262))]])
            weights_y.requires_grad=True
            weights_y = weights_y.view(1,1,3,3).repeat(450,1,1,1).to(device)
            dphi_dy = torch.nn.functional.conv2d(predictions.permute(0,2,3,1), weights_y, groups=450).permute(0,3,1,2).reshape(args.batch_size,3,1,450,190)
            
            dphi_dy = dphi_dy[:,:,:,1:-1,:]
            
            predictions_pinns = torch.zeros((args.batch_size*3,1,448,190)).to(device)
            predictions_pinns[torch.arange(0,args.batch_size*3,3),:,:,:] = (dphi_dy[:,2,:,:,:]-dphi_dz[:,1,:,:,:])
            predictions_pinns[torch.arange(1,args.batch_size*3,3),:,:,:] = (dphi_dz[:,0,:,:,:]-dphi_dx[:,2,:,:,:])
            predictions_pinns[torch.arange(2,args.batch_size*3,3),:,:,:] = (dphi_dx[:,1,:,:,:]-dphi_dy[:,0,:,:,:])

            loss = loss_function(predictions_pinns.float(), inputs.reshape(args.batch_size,3,3,450,192).permute(0,2,1,3,4).reshape(args.batch_size*3,3,450,192)[:,1:-1,1:-1,1:-1].float().to(device))   # Compute loss function
...
# start testing loop
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            dphi_dx = dphi_dx.reshape(2,3,3,448,190).permute(0,2,1,3,4)
            dphi_dx = dphi_dx[:,:,1:-1,:,:]
            ...
            dphi_dz = dphi_dz.reshape(2,3,3,448,190).permute(0,2,1,3,4)
            dphi_dz = dphi_dz[:,:,1:-1,:,:]
            
            ## for the convolution in y-dir (wall-normal), the tensor has to be transposed first
            predictions = predictions.reshape(2,3,3,450,192).permute(0,2,1,3,4).reshape(2*3,3,450,192)
            weights_y = torch.tensor([[0.,0.,0.],[-0.0262/(0.0255*(0.0255+0.0262)),
                        (0.0262-0.0255)/(0.0262*0.0255),0.0255/(0.0262*(0.0255+0.0262))],[0.,0.,0.]])
            weights_y = weights_y.view(1,1,3,3).repeat(450,1,1,1).to(device)
            dphi_dy = torch.nn.functional.conv2d(predictions.permute(0,2,3,1), weights_y, groups=450).permute(0,3,1,2).reshape(2,3,1,450,190)
            dphi_dy = dphi_dy[:,:,:,1:-1,:]
            
            
            predictions_pinns = torch.zeros((2*3,1,448,190)).to(device)
            predictions_pinns[torch.arange(0,2*3,3),:,:,:] = (dphi_dy[:,2,:,:,:]-dphi_dz[:,1,:,:,:])
            predictions_pinns[torch.arange(1,2*3,3),:,:,:] = (dphi_dz[:,0,:,:,:]-dphi_dx[:,2,:,:,:])
            predictions_pinns[torch.arange(2,2*3,3),:,:,:] = (dphi_dx[:,1,:,:,:]-dphi_dy[:,0,:,:,:])
            
            loss = loss_function(predictions_pinns.float(), inputs.reshape(2,3,3,450,192).permute(0,2,1,3,4).reshape(2*3,3,450,192)[:,1:-1,1:-1,1:-1].float().to(device))
            
            test_loss+= loss.item()/inputs.shape[0]
            # mean squared prediction difference (Jin et al., PoF 30, 2018, Eq. 7)
            mean_sqr_diff.append(\
                torch.mean(torch.square(predictions_pinns.float()-inputs.reshape(2,3,3,450,192).permute(0,2,1,3,4).reshape(2*3,3,450,192)[:,1:-1,1:-1,1:-1].float())).item())
            count+=1
    ...
    if grank==0:
        ...
        if not args.testrun or not args.benchrun:
            plot_scatter(inputs.reshape(2,3,3,450,192).permute(0,2,1,3,4).reshape(2*3,3,450,192)[:,1:-1,1:-1,1:-1][0][0].cpu().detach().numpy(), predictions_pinns[0][0].cpu().detach().numpy(), 'test')
            plot_scatter(inputs.reshape(2,3,3,450,192).permute(0,2,1,3,4).reshape(2*3,3,450,192)[:,1:-1,1:-1,1:-1][1][0].cpu().detach().numpy(), predictions_pinns[1][0].cpu().detach().numpy(), 'test')
            plot_scatter(inputs.reshape(2,3,3,450,192).permute(0,2,1,3,4).reshape(2*3,3,450,192)[:,1:-1,1:-1,1:-1][3][0].cpu().detach().numpy(), predictions_pinns[3][0].cpu().detach().numpy(), 'test')
