...
def collate_batch(batch):
    imgs = [item[0][0].reshape(int(item[0][0].shape[0]/89), 89, item[0][0].shape[1], \
        item[0][0].shape[2], item[0][0].shape[3]) for item in batch]
    par_wid = [torch.Tensor([item[0][1]]).repeat(int(item[0][0].shape[0]/89)) for item in batch]
    par_wl = [torch.Tensor([item[0][2]]).repeat(int(item[0][0].shape[0]/89)) for item in batch]
    par_T = [torch.Tensor([item[0][3]]).repeat(int(item[0][0].shape[0]/89)) for item in batch]
    par_amp = [torch.Tensor([item[0][4]]).repeat(int(item[0][0].shape[0]/89)) for item in batch]
    par_cd = [torch.Tensor([item[0][5]]).repeat(int(item[0][0].shape[0]/89)) for item in batch]
    par_Pnet = [torch.Tensor([item[0][6]]).repeat(int(item[0][0].shape[0]/89)) for item in batch]
...
class nn_reg(nn.Module):
    def __init__(self):
        ...
        self.reg_conv1 = nn.Conv3d(3, 8, kernel_size=3, stride=2, padding=1, bias=False)
        self.reg_bn1 = nn.BatchNorm3d(8)
        self.reg_conv2 = nn.Conv3d(8, 16, kernel_size=3, stride=2, padding=1, bias=False)
        self.reg_bn2 = nn.BatchNorm3d(16)
        self.reg_conv3 = nn.Conv3d(16, 32, kernel_size=3, stride=2, padding=1, bias=False)
        self.reg_bn3 = nn.BatchNorm3d(32)
        self.reg_conv4 = nn.Conv3d(32, 32, kernel_size=3, stride=2, padding=1, bias=False)
        self.reg_bn4 = nn.BatchNorm3d(32)
        self.reg_fc1 = nn.Linear(36864, 2048)
        ...
        #self.conv4 = nn.Conv3d(64, 16, kernel_size=3, stride=2, padding=1, bias=False)
...
def save_to_hdf5(inps, preds, width, wavelength, period, amplitude, count):
    hf5 = h5py.File('/p/scratch/raise-ctp1/T31_LD_CAE_recon/TBL_sample'+str(count)+'.h5', 'w')
    ...
    if grank==0:
        ...
        if not args.testrun or not args.benchrun:
            ...
            plot_scatter(inps[0][0][0].cpu().detach().numpy(), 
                    predictions[1][0][0][0].cpu().detach().numpy(), 'test')
