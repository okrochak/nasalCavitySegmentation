...
# train loop
def train(model, device, train_loader, optimizer, epoch, grank, gwsize):
    model.train()
    t_list = []
    loss_acc=0
    if grank==0:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if args.benchrun and epoch==args.epochs:
            ...
            with torch.autograd.profiler.profile(use_cuda=args.cuda, profile_memory=True) as prof:
                loss_acc = train(distrib_model, device, train_loader, optimizer, epoch, grank, gwsize)
        else:
            loss_acc = train(distrib_model, device, train_loader, optimizer, epoch, grank, gwsize)

        # testing
        acc_test = test(distrib_model, device, test_loader, grank, gwsize)
