...
# author: EI
# version: 220222a
# NOTE: apex might be depreciated, if not, this script will be merged with DDP_pytorch_AT.py 
# CUDA/11.3 is needed to compile APEX, use these commands in the Python Env:
# $git clone https://github.com/NVIDIA/apex
# $cd apex
# $pip3 install --global-option="--cpp_ext" --global-option="--cuda_ext" ./ --no-cache-dir
# to turn on Apex, simply add --apex flag
...
def pars_ini():
    ...
    # NVIDIA APEX optimizations
    parser.add_argument('--apex', action='store_true', default=False,
                        help='turn o NVIDIA Apex optimizations (default: False)')
...
def save_state(epoch,distrib_model,loss_acc,optimizer,res_name,grank,gwsize,is_best):
    ...
    if any(is_best_m):
        ...
        if args.apex:
            ...
            if grank==0:
                print(f'WARNING: state with Apex optimizations are saved!')
                print(f'restart only with --apex flag')
        else:
            state = {'epoch': epoch + 1,
                 'state_dict': distrib_model.state_dict(),
                 'best_acc': loss_acc,
...
# deterministic testrun
    ...
    if grank==0:
        ...
        print('DEBUG: optimization parsers:')
        print('DEBUG: args.apex:',args.apex,'\n')
...
# load datasets
    ...
    turb_data = datasets.DatasetFolder(args.data_dir+'trainfolder/Width3000',\
        loader=hdf5_loader, transform=transform, extensions='.hdf5')
    test_data = datasets.DatasetFolder(args.data_dir+'testfolder/Width3000',\
         loader=hdf5_loader, transform=transform, extensions='.hdf5')
...
# distribute model to workers
    if args.cuda and not args.apex:
        distrib_model = torch.nn.parallel.DistributedDataParallel(model,\
            device_ids=[device], output_device=device)
    elif not args.cuda and not args.apex:
        distrib_model = torch.nn.parallel.DistributedDataParallel(model)
...
# optimizer
    if args.cuda and args.apex:
        ...
        # opt_level="O0" fast - few optimizations 
        # opt_level="O1" does not work - some ops in FP16
        # opt_level="O2" slow - all ops in FP16 
        model, optimizer = apex.amp.initialize(model, optimizer, opt_level="O0")
        distrib_model = apex.parallel.DistributedDataParallel(model)
    else:
