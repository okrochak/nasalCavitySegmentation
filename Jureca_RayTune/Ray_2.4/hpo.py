...
# @file hpo.py
#
# @brief Optimizing the hyperparameters of a ResNet18 trained on the CIFAR-10 dataset with Ray Tune libray
# and the ASHA, BOHB and PBT algorithm.
#
# @section hpo description
...
# For ASHA: Trials with low performance (in terms of test set acuracy) are terminated early
# with the ASHA aglorithm.
# For BOHB: Trials with low performance (in terms of test set acuracy) are terminated early
# and their resources are assigned to new samples with the BOHB aglorithm.
# For PBT: Trials with low performance (in terms of test set acuracy) copy the hyperparameters
# of better performing trials and apply mutations with the PBT aglorithm.
# For RAND: Random Search termination (no algorithm)
#
#
...
def parsIni():
    ...
    @param --num-samples        #samples
    @param --max-iterations     max. iteration
    @param --ngpus              parallel-workers per trial
    @param --scheduler          schedulers, ASHA, BOHB, PBT, RAND (no algorithm)
    @param --data-dir           dataset location
    """
    parser = argparse.ArgumentParser(description='HPO Suite for AI4HPC')
    ...
    parser.add_argument('--ngpus', type=int, default=1, metavar='N',
                    help='number of GPUs used in a single trial (default: 1)')
...
def train_cifar(config):
    ...
    if session.get_checkpoint() and args.scheduler == "PBT":
        step = 1
...
def main(args):
    ...
    if (args.scheduler == "ASHA"):
        ...
        # modify search algorithm for BOHB
        search_alg = TuneBOHB(seed=42)

    elif (args.scheduler == "PBT"):
        from ray.tune.schedulers import PopulationBasedTraining
        from ray.air import session, Checkpoint
        # define the mutation config
        mutation_config = {"lr": tune.loguniform(10e-5, 1),}

        # Population Based Training
        scheduler = PopulationBasedTraining(
            # time attribute
            time_attr="training_iteration",
            # intervals at that perturbations occur,
            perturbation_interval=1,
            # specification of hyperparameter mutatation search space (can be different than original search space!)
            hyperparam_mutations={"train_loop_config": mutation_config},
            """ the parameters of the top quantile_fraction percentage trials are transfered to the bottom
            quantile_fraction percentage of trials"""
            quantile_fraction=0.33,
            # probability to resample from original hyperparameter search space
            resample_probability=0,
            )

    elif (args.scheduler == "RAND"):
        # random scheduler
        scheduler = None
    ...
    if (args.scheduler == "PBT"):
        #checkpointing
        run_config.checkpoint_config=ray.air.CheckpointConfig(checkpoint_score_attribute="test_acc")
